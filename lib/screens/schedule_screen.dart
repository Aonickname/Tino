  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Colors.white,
        elevation: 0,
        title: Text("지난 회의록 보기", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.black)),
        actions: [
          OutlinedButton(
        ...
        children: [
          ...
          Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text("${_selectedDay.year}년 ${_selectedDay.month}월 ${_selectedDay.day}일 회의 내역",
                    style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                SizedBox(height: 10),
                _meetingEvents.containsKey(normalizeDate(_selectedDay))
                    ? Column(
                  children: _meetingEvents[normalizeDate(_selectedDay)]!.map((event) => ListTile(
